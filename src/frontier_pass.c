...
static const union AnimCmd *const sAnims_Medal[] =
    {
        ...
        [CURSOR_AREA_SYMBOL_DOME -
            CURSOR_AREA_SYMBOL] = sAnim_Frame2,
        [CURSOR_AREA_SYMBOL_PALACE -
            CURSOR_AREA_SYMBOL] = sAnim_Frame3,
        [CURSOR_AREA_SYMBOL_ARENA -
            CURSOR_AREA_SYMBOL] = sAnim_Frame4,
        [CURSOR_AREA_SYMBOL_FACTORY -
            CURSOR_AREA_SYMBOL] = sAnim_Frame5,
        [CURSOR_AREA_SYMBOL_PIKE -
            CURSOR_AREA_SYMBOL] = sAnim_Frame6,
        [CURSOR_AREA_SYMBOL_PYRAMID -
            CURSOR_AREA_SYMBOL] = sAnim_Frame7};
...
static u8 GetCursorAreaFromCoords(s16 x, s16 y)
{
    ...
    {
        if (sPassAreasLayout[i].yStart <= y && sPassAreasLayout[i].yEnd >= y && sPassAreasLayout[i].xStart <= x && sPassAreasLayout[i].xEnd >= x)
        {
...
{
    // In Battle Tower
    if ((mapNum >= MAP_NUM(HOENN_BATTLE_FRONTIER_BATTLE_TOWER_LOBBY) && mapNum <= MAP_NUM(HOENN_BATTLE_FRONTIER_BATTLE_TOWER_BATTLE_ROOM)) || (mapNum >= MAP_NUM(HOENN_BATTLE_FRONTIER_BATTLE_TOWER_MULTI_PARTNER_ROOM) && mapNum <= MAP_NUM(HOENN_BATTLE_FRONTIER_BATTLE_TOWER_MULTI_BATTLE_ROOM)))
        return FRONTIER_FACILITY_TOWER + 1;

    // In Battle Dome
    else if (mapNum == MAP_NUM(HOENN_BATTLE_FRONTIER_BATTLE_DOME_LOBBY) || mapNum == MAP_NUM(HOENN_BATTLE_FRONTIER_BATTLE_DOME_CORRIDOR) || mapNum == MAP_NUM(HOENN_BATTLE_FRONTIER_BATTLE_DOME_PRE_BATTLE_ROOM) || mapNum == MAP_NUM(HOENN_BATTLE_FRONTIER_BATTLE_DOME_BATTLE_ROOM))
        return FRONTIER_FACILITY_DOME + 1;

    // In Battle Palace
    else if (mapNum == MAP_NUM(HOENN_BATTLE_FRONTIER_BATTLE_PALACE_LOBBY) || mapNum == MAP_NUM(HOENN_BATTLE_FRONTIER_BATTLE_PALACE_CORRIDOR) || mapNum == MAP_NUM(HOENN_BATTLE_FRONTIER_BATTLE_PALACE_BATTLE_ROOM))
        return FRONTIER_FACILITY_PALACE + 1;

    // In Battle Arena
    else if (mapNum == MAP_NUM(HOENN_BATTLE_FRONTIER_BATTLE_ARENA_LOBBY) || mapNum == MAP_NUM(HOENN_BATTLE_FRONTIER_BATTLE_ARENA_CORRIDOR) || mapNum == MAP_NUM(HOENN_BATTLE_FRONTIER_BATTLE_ARENA_BATTLE_ROOM))
        return FRONTIER_FACILITY_ARENA + 1;

    // In Battle Factory
    else if (mapNum == MAP_NUM(HOENN_BATTLE_FRONTIER_BATTLE_FACTORY_LOBBY) || mapNum == MAP_NUM(HOENN_BATTLE_FRONTIER_BATTLE_FACTORY_PRE_BATTLE_ROOM) || mapNum == MAP_NUM(HOENN_BATTLE_FRONTIER_BATTLE_FACTORY_BATTLE_ROOM))
        return FRONTIER_FACILITY_FACTORY + 1;

    // In Battle Pike
    else if (mapNum == MAP_NUM(HOENN_BATTLE_FRONTIER_BATTLE_PIKE_LOBBY) || mapNum == MAP_NUM(HOENN_BATTLE_FRONTIER_BATTLE_PIKE_CORRIDOR) || mapNum == MAP_NUM(HOENN_BATTLE_FRONTIER_BATTLE_PIKE_THREE_PATH_ROOM) || mapNum == MAP_NUM(HOENN_BATTLE_FRONTIER_BATTLE_PIKE_ROOM_NORMAL) || mapNum == MAP_NUM(HOENN_BATTLE_FRONTIER_BATTLE_PIKE_ROOM_FINAL) || mapNum == MAP_NUM(HOENN_BATTLE_FRONTIER_BATTLE_PIKE_ROOM_WILD_MONS))
        return FRONTIER_FACILITY_PIKE + 1;

    // In Battle Pyramid
    else if (mapNum == MAP_NUM(HOENN_BATTLE_FRONTIER_BATTLE_PYRAMID_LOBBY) || mapNum == MAP_NUM(HOENN_BATTLE_FRONTIER_BATTLE_PYRAMID_FLOOR) || mapNum == MAP_NUM(HOENN_BATTLE_FRONTIER_BATTLE_PYRAMID_TOP))
...
static void InitFrontierMapSprites(void)
{
    ...
    if (id == MAPSEC_BATTLE_FRONTIER || id == MAPSEC_ARTISAN_CAVE)
    {
        ...
        if (mapNum == MAP_NUM(HOENN_BATTLE_FRONTIER_OUTSIDE_WEST) || (mapNum == MAP_NUM(HOENN_BATTLE_FRONTIER_OUTSIDE_EAST) && (x = 55)))
        {
        ...
        else
        {
            ...
            {
                // Handle Artisan Cave.
                if (gSaveBlock1Ptr->escapeWarp.mapNum == MAP_NUM(HOENN_BATTLE_FRONTIER_OUTSIDE_EAST))
