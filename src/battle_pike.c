...
static bool8 DoesTypePreventStatus(u16 species, u32 status)
{
    ...
    {
    case STATUS1_TOXIC_POISON:
        if (gSpeciesInfo[species].types[0] == TYPE_STEEL || gSpeciesInfo[species].types[0] == TYPE_POISON || gSpeciesInfo[species].types[1] == TYPE_STEEL || gSpeciesInfo[species].types[1] == TYPE_POISON)
            ret = TRUE;
        break;
    case STATUS1_FREEZE:
        ...
        break;
    case STATUS1_PARALYSIS:
        if (gSpeciesInfo[species].types[0] == TYPE_GROUND || gSpeciesInfo[species].types[0] == TYPE_ELECTRIC || gSpeciesInfo[species].types[1] == TYPE_GROUND || gSpeciesInfo[species].types[1] == TYPE_ELECTRIC)
            ret = TRUE;
        break;
    case STATUS1_BURN:
    ...
    do
    {
        ...
        if (status != sStatusFlags)
        {
            ...
            for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
            {
                ...
                if (GetAilmentFromStatus(GetMonData(mon, MON_DATA_STATUS)) == AILMENT_NONE && GetMonData(mon, MON_DATA_HP) != 0)
                {
                    j++;
...
{
    return gMapHeader.mapLayoutId == LAYOUT_HOENN_BATTLE_FRONTIER_BATTLE_PIKE_THREE_PATH_ROOM || gMapHeader.mapLayoutId == LAYOUT_HOENN_BATTLE_FRONTIER_BATTLE_PIKE_ROOM_NORMAL || gMapHeader.mapLayoutId == LAYOUT_HOENN_BATTLE_FRONTIER_BATTLE_PIKE_ROOM_WILD_MONS || gMapHeader.mapLayoutId == LAYOUT_HOENN_BATTLE_FRONTIER_BATTLE_PIKE_ROOM_UNUSED;
}
