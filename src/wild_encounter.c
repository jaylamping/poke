...
static bool8 CheckFeebas(void)
{
    ...
    if (gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(HOENN_ROUTE119) && gSaveBlock1Ptr->location.mapNum == MAP_NUM(HOENN_ROUTE119))
    {
...
static u16 GetCurrentMapWildMonHeaderId(void)
{
    ...
    for (i = 0;; i++)
    {
        ...
        if (wildHeader->mapGroup == MAP_GROUP(HOENN_UNDEFINED))
            break;

        if (gWildMonHeaders[i].mapGroup == gSaveBlock1Ptr->location.mapGroup &&
            gWildMonHeaders[i].mapNum == gSaveBlock1Ptr->location.mapNum)
        {
            if (gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(HOENN_ALTERING_CAVE) &&
                gSaveBlock1Ptr->location.mapNum == MAP_NUM(HOENN_ALTERING_CAVE))
            {
...
static bool8 TryGenerateWildMon(const struct WildPokemonInfo *wildMonInfo, u8 area, u8 flags)
{
    ...
    if (flags & WILD_CHECK_REPEL && !IsWildLevelAllowedByRepel(level))
        return FALSE;
    if (gMapHeader.mapLayoutId != LAYOUT_HOENN_BATTLE_FRONTIER_BATTLE_PIKE_ROOM_WILD_MONS && flags & WILD_CHECK_KEEN_EYE && !IsAbilityAllowingEncounter(level))
        return FALSE;
...
static bool8 WildEncounterCheck(u32 encounterRate, bool8 ignoreAbility)
{
    ...
    if (!ignoreAbility && !GetMonData(&gPlayerParty[0], MON_DATA_SANITY_IS_EGG))
    {
        ...
        if (ability == ABILITY_STENCH && gMapHeader.mapLayoutId == LAYOUT_HOENN_BATTLE_FRONTIER_BATTLE_PYRAMID_FLOOR)
...
{
    if (gSaveBlock1Ptr->location.mapGroup != MAP_GROUP(HOENN_SOOTOPOLIS_CITY) || gSaveBlock1Ptr->location.mapNum != MAP_NUM(HOENN_SOOTOPOLIS_CITY))
    {
        return FALSE;
    }
...
bool8 StandardWildEncounter(u16 curMetatileBehavior, u16 prevMetatileBehavior)
{
    ...
    {
        if (gMapHeader.mapLayoutId == LAYOUT_HOENN_BATTLE_FRONTIER_BATTLE_PIKE_ROOM_WILD_MONS)
        {
