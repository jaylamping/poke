...
#define NUM_LAYOUT_HOENN_OFFSETS 8
...
struct PyramidFloorTemplate
{
    ...
    u8 trainerPositions;
    u8 runMultiplier;
    u8 layoutOffsets[NUM_LAYOUT_HOENN_OFFSETS];
};
    ...
    for (y = 0; y < 32; map += 47, y++)
    {
        for (x = 0; x < 32; x++)
        {
            if ((map[x] & MAPGRID_METATILE_ID_MASK) == METATILE_BattlePyramid_Exit)
            {
                ...
                if (x >= minDistanceForExitHint || x <= -minDistanceForExitHint || y >= minDistanceForExitHint || y <= -minDistanceForExitHint || defaultHintType == HINT_EXIT_DIRECTION)
                {
                    if (x > 0 && y > 0)
                    {
                        if (x >= y)
...
u8 InBattlePyramid(void)
{
    if (gMapHeader.mapLayoutId == LAYOUT_HOENN_BATTLE_FRONTIER_BATTLE_PYRAMID_FLOOR)
        return 1;
    else if (gMapHeader.mapLayoutId == LAYOUT_HOENN_BATTLE_FRONTIER_BATTLE_PYRAMID_TOP)
        return 2;
    else
        return FALSE;
}
...
{
    return gMapHeader.mapLayoutId == LAYOUT_HOENN_BATTLE_FRONTIER_BATTLE_PYRAMID_FLOOR || gMapHeader.mapLayoutId == LAYOUT_HOENN_BATTLE_FRONTIER_BATTLE_PYRAMID_TOP;
}
...
void GenerateBattlePyramidFloorLayout(u16 *backupMapData, bool8 setPlayerPosition)
{
    ...
    for (i = 0; i < NUM_PYRAMID_FLOOR_SQUARES; i++)
    {
        u16 *map;
        int yOffset, xOffset;
        const struct MapLayout *mapLayout = gMapLayouts[floorLayoutOffsets[i] + LAYOUT_HOENN_BATTLE_FRONTIER_BATTLE_PYRAMID_FLOOR];
...
static bool8 TrySetPyramidObjectEventPositionAtCoords(u8 objType, u8 x, u8 y, u8 *floorLayoutOffsets, u8 squareId, u8 objectEventId)
{
    ...
    mapHeader = Overworld_GetMapHeaderByGroupAndId(MAP_GROUP(HOENN_BATTLE_PYRAMID_SQUARE01), floorLayoutOffsets[squareId] + MAP_NUM(HOENN_BATTLE_PYRAMID_SQUARE01));
...
static void GetPyramidFloorLayoutOffsets(u8 *layoutOffsets)
{
    ...
    for (i = 0; i < NUM_PYRAMID_FLOOR_SQUARES; i++)
    {
        layoutOffsets[i] = sPyramidFloorTemplates[id].layoutOffsets[MOD(rand, NUM_LAYOUT_HOENN_OFFSETS)];
        rand >>= 3;
        if (i == 7)
        {
