...
static const struct WindowTemplate sUIWindowTemplates[] = {
    [FCWINDOWID_LIST] = {
        ...
        .paletteNum = 15,
        .baseBlock = 20},
    [FCWINDOWID_UIHELP] = {.bg = 0, .tilemapLeft = 6, .tilemapTop = 0, .width = 24, .height = 2, .paletteNum = 15, .baseBlock = 20 + 8 * 10},
    [FCWINDOWID_MSGBOX] = {.bg = 0, .tilemapLeft = 2, .tilemapTop = 15, .width = 26, .height = 4, .paletteNum = 15, .baseBlock = 20 + 8 * 10 + 24 * 2},
    [FCWINDOWID_ICONDESC] = {.bg = 0, .tilemapLeft = 15, .tilemapTop = 10, .width = 11, .height = 4, .paletteNum = 15, .baseBlock = 20 + 8 * 10 + 24 * 2 + 26 * 4},
    DUMMY_WIN_TEMPLATE};
...
void UpdatePickStateFromSpecialVar8005(void)
{
    if (gSpecialVar_0x8004 < NUM_FAMECHECKER_PERSONS && gSpecialVar_0x8005 < 3)
    {
        if (gSpecialVar_0x8005 == FCPICKSTATE_NO_DRAW)
            return;
        if (gSpecialVar_0x8005 == FCPICKSTATE_SILHOUETTE && gSaveBlock1Ptr->fameChecker[gSpecialVar_0x8004].pickState == FCPICKSTATE_COLORED)
            return;
...
static void DestroyPersonPicSprite(u8 taskId, u16 who)
{
    ...
    if (who == sFameCheckerData->numUnlockedPersons - 1)
        who_copy = who - 1;
    if (sFameCheckerData->unlockedPersons[who_copy] == FAMECHECKER_DAISY || sFameCheckerData->unlockedPersons[who_copy] == FAMECHECKER_MRFUJI || sFameCheckerData->unlockedPersons[who_copy] == FAMECHECKER_OAK || sFameCheckerData->unlockedPersons[who_copy] == FAMECHECKER_BILL)
