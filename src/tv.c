...
void UpdateTVScreensOnMap(int width, int height)
{
    ...
    default:
        if (gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(HOENN_LILYCOVE_CITY_COVE_LILY_MOTEL_1F) && gSaveBlock1Ptr->location.mapNum == MAP_NUM(HOENN_LILYCOVE_CITY_COVE_LILY_MOTEL_1F))
        {
...
void TryPutSmartShopperOnAir(void)
{
    ...
    if (!(gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(HOENN_TRAINER_HILL_ENTRANCE) && gSaveBlock1Ptr->location.mapNum == MAP_NUM(HOENN_TRAINER_HILL_ENTRANCE)) && !(gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(HOENN_BATTLE_FRONTIER_MART) && gSaveBlock1Ptr->location.mapNum == MAP_NUM(HOENN_BATTLE_FRONTIER_MART)) && !rbernoulli(1, 3))
    {
        sCurTVShowSlot = FindFirstEmptyRecordMixTVShowSlot(gSaveBlock1Ptr->tvShows);
...
static bool8 ShouldApplyPokeNewsEffect(u8 newsKind)
{
    ...
    case POKENEWS_SLATEPORT:
        if (gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(HOENN_SLATEPORT_CITY) && gSaveBlock1Ptr->location.mapNum == MAP_NUM(HOENN_SLATEPORT_CITY) && gSpecialVar_LastTalked == LOCALID_SLATEPORT_ENERGY_GURU)
            return TRUE;
        return FALSE;
    case POKENEWS_LILYCOVE:
        if (gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(HOENN_LILYCOVE_CITY_DEPARTMENT_STORE_ROOFTOP) && gSaveBlock1Ptr->location.mapNum == MAP_NUM(HOENN_LILYCOVE_CITY_DEPARTMENT_STORE_ROOFTOP))
            return TRUE;
        return FALSE;
    }
    return TRUE;
...
u8 CheckForPlayersHouseNews(void)
{
    ...
    if (gSaveBlock1Ptr->location.mapGroup != MAP_GROUP(HOENN_LITTLEROOT_TOWN_BRENDANS_HOUSE_1F))
        return PLAYERS_HOUSE_TV_NONE;

    // Check if not in player's house (dependent on gender)
    if (gSaveBlock2Ptr->playerGender == MALE)
    {
        if (gSaveBlock1Ptr->location.mapNum != MAP_NUM(HOENN_LITTLEROOT_TOWN_BRENDANS_HOUSE_1F))
            return PLAYERS_HOUSE_TV_NONE;
    }
    else
    {
        if (gSaveBlock1Ptr->location.mapNum != MAP_NUM(HOENN_LITTLEROOT_TOWN_MAYS_HOUSE_1F))
            return PLAYERS_HOUSE_TV_NONE;
    }
