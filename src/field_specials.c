...
{
    if (gLastUsedWarp.mapNum == MAP_NUM(HOENN_ROUTE110_SEASIDE_CYCLING_ROAD_NORTH_ENTRANCE) && gLastUsedWarp.mapGroup == MAP_GROUP(HOENN_ROUTE110_SEASIDE_CYCLING_ROAD_NORTH_ENTRANCE))
        return;
...
u8 GetSSTidalLocation(s8 *mapGroup, s8 *mapNum, s16 *x, s16 *y)
{
    ...
    case SS_TIDAL_DEPART_SLATEPORT:
        ...
        {
            *mapNum = MAP_NUM(HOENN_ROUTE134);
            *x = *varCruiseStepCount + 19;
        }
        else if (*varCruiseStepCount < 140)
        {
            *mapNum = MAP_NUM(HOENN_ROUTE133);
            *x = *varCruiseStepCount - 60;
        }
        else
        {
            *mapNum = MAP_NUM(HOENN_ROUTE132);
    ...
    *mapGroup = MAP_GROUP(HOENN_ROUTE132);
    *y = 20;
...
void CableCarWarp(void)
{
    if (gSpecialVar_0x8004 != 0)
        SetWarpDestination(MAP_GROUP(HOENN_ROUTE112_CABLE_CAR_STATION), MAP_NUM(HOENN_ROUTE112_CABLE_CAR_STATION), WARP_ID_NONE, 6, 4);
    else
        SetWarpDestination(MAP_GROUP(HOENN_MT_CHIMNEY_CABLE_CAR_STATION), MAP_NUM(HOENN_MT_CHIMNEY_CABLE_CAR_STATION), WARP_ID_NONE, 6, 4);
}
...
{
    if (!FlagGet(FLAG_RUSTURF_TUNNEL_OPENED) && gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(HOENN_RUSTURF_TUNNEL) && gSaveBlock1Ptr->location.mapNum == MAP_NUM(HOENN_RUSTURF_TUNNEL))
    {
...
{
    if (gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(HOENN_BATTLE_FRONTIER_BATTLE_TOWER_MULTI_PARTNER_ROOM) && gSaveBlock1Ptr->location.mapNum == MAP_NUM(HOENN_BATTLE_FRONTIER_BATTLE_TOWER_MULTI_PARTNER_ROOM) &&
...
void SetDeptStoreFloor(void)
{
    ...
    {
    case MAP_NUM(HOENN_LILYCOVE_CITY_DEPARTMENT_STORE_1F):
        deptStoreFloor = DEPT_STORE_FLOORNUM_1F;
        break;
    case MAP_NUM(HOENN_LILYCOVE_CITY_DEPARTMENT_STORE_2F):
        deptStoreFloor = DEPT_STORE_FLOORNUM_2F;
        break;
    case MAP_NUM(HOENN_LILYCOVE_CITY_DEPARTMENT_STORE_3F):
        deptStoreFloor = DEPT_STORE_FLOORNUM_3F;
        break;
    case MAP_NUM(HOENN_LILYCOVE_CITY_DEPARTMENT_STORE_4F):
        deptStoreFloor = DEPT_STORE_FLOORNUM_4F;
        break;
    case MAP_NUM(HOENN_LILYCOVE_CITY_DEPARTMENT_STORE_5F):
        deptStoreFloor = DEPT_STORE_FLOORNUM_5F;
        break;
    case MAP_NUM(HOENN_LILYCOVE_CITY_DEPARTMENT_STORE_ROOFTOP):
...
u16 GetDeptStoreDefaultFloorChoice(void)
{
    ...
    if (gSaveBlock1Ptr->dynamicWarp.mapGroup == MAP_GROUP(HOENN_LILYCOVE_CITY_DEPARTMENT_STORE_1F))
    {
        switch (gSaveBlock1Ptr->dynamicWarp.mapNum)
        {
        case MAP_NUM(HOENN_LILYCOVE_CITY_DEPARTMENT_STORE_5F):
...
{
    if (gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(HOENN_TRAINER_HILL_ENTRANCE) && gSaveBlock1Ptr->location.mapNum == MAP_NUM(HOENN_TRAINER_HILL_ENTRANCE))
        return FALSE;
...
static void ChangeDeoxysRockLevel(u8 rockLevel)
{
    ...
    gFieldEffectArguments[1] = MAP_NUM(HOENN_BIRTH_ISLAND_EXTERIOR);
    gFieldEffectArguments[2] = MAP_GROUP(HOENN_BIRTH_ISLAND_EXTERIOR);
...
{
    u16 stepCount = VarGet(VAR_DEOXYS_ROCK_STEP_COUNT);
    if (gSaveBlock1Ptr->location.mapNum == MAP_NUM(HOENN_BIRTH_ISLAND_EXTERIOR) && gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(HOENN_BIRTH_ISLAND_EXTERIOR))
    {
        if (++stepCount > 99)
            VarSet(VAR_DEOXYS_ROCK_STEP_COUNT, 0);
        else
...
bool32 GetAbnormalWeatherMapNameAndType(void)
{
    static const u8 sAbnormalWeatherMapNumbers[] = {
        MAP_NUM(HOENN_ROUTE114),
        MAP_NUM(HOENN_ROUTE114),
        MAP_NUM(HOENN_ROUTE115),
        MAP_NUM(HOENN_ROUTE115),
        MAP_NUM(HOENN_ROUTE116),
        MAP_NUM(HOENN_ROUTE116),
        MAP_NUM(HOENN_ROUTE118),
        MAP_NUM(HOENN_ROUTE118),
        MAP_NUM(HOENN_ROUTE105),
        MAP_NUM(HOENN_ROUTE105),
        MAP_NUM(HOENN_ROUTE125),
        MAP_NUM(HOENN_ROUTE125),
        MAP_NUM(HOENN_ROUTE127),
        MAP_NUM(HOENN_ROUTE127),
        MAP_NUM(HOENN_ROUTE129),
        MAP_NUM(HOENN_ROUTE129)};
...
bool8 AbnormalWeatherHasExpired(void)
{
    ...
    {
        VarSet(VAR_ABNORMAL_WEATHER_STEP_COUNTER, 0);
        if (gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(HOENN_UNDERWATER_MARINE_CAVE))
        {
            switch (gSaveBlock1Ptr->location.mapNum)
            {
            case MAP_NUM(HOENN_UNDERWATER_MARINE_CAVE):
            case MAP_NUM(HOENN_MARINE_CAVE_ENTRANCE):
            case MAP_NUM(HOENN_MARINE_CAVE_END):
            case MAP_NUM(HOENN_TERRA_CAVE_ENTRANCE):
            case MAP_NUM(HOENN_TERRA_CAVE_END):
        ...
        if (gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(HOENN_UNDERWATER_ROUTE127))
        {
            switch (gSaveBlock1Ptr->location.mapNum)
            {
            case MAP_NUM(HOENN_UNDERWATER_ROUTE127):
            case MAP_NUM(HOENN_UNDERWATER_ROUTE129):
            case MAP_NUM(HOENN_UNDERWATER_ROUTE105):
            case MAP_NUM(HOENN_UNDERWATER_ROUTE125):
                VarSet(VAR_SHOULD_END_ABNORMAL_WEATHER, 1);
                return FALSE;
            default:
                break;
...
u32 GetMartEmployeeObjectEventId(void)
{
    static const u8 sPokeMarts[][3] =
        {
            {MAP_GROUP(HOENN_OLDALE_TOWN_MART), MAP_NUM(HOENN_OLDALE_TOWN_MART), LOCALID_OLDALE_MART_CLERK},
            {MAP_GROUP(HOENN_LAVARIDGE_TOWN_MART), MAP_NUM(HOENN_LAVARIDGE_TOWN_MART), LOCALID_LAVARIDGE_MART_CLERK},
            {MAP_GROUP(HOENN_FALLARBOR_TOWN_MART), MAP_NUM(HOENN_FALLARBOR_TOWN_MART), LOCALID_FALLARBOR_MART_CLERK},
            {MAP_GROUP(HOENN_VERDANTURF_TOWN_MART), MAP_NUM(HOENN_VERDANTURF_TOWN_MART), LOCALID_VERDANTURF_MART_CLERK},
            {MAP_GROUP(HOENN_PETALBURG_CITY_MART), MAP_NUM(HOENN_PETALBURG_CITY_MART), LOCALID_PETALBURG_MART_CLERK},
            {MAP_GROUP(HOENN_SLATEPORT_CITY_MART), MAP_NUM(HOENN_SLATEPORT_CITY_MART), LOCALID_SLATEPORT_MART_CLERK},
            {MAP_GROUP(HOENN_MAUVILLE_CITY_MART), MAP_NUM(HOENN_MAUVILLE_CITY_MART), LOCALID_MAUVILLE_MART_CLERK},
            {MAP_GROUP(HOENN_RUSTBORO_CITY_MART), MAP_NUM(HOENN_RUSTBORO_CITY_MART), LOCALID_RUSTBORO_MART_CLERK},
            {MAP_GROUP(HOENN_FORTREE_CITY_MART), MAP_NUM(HOENN_FORTREE_CITY_MART), LOCALID_FORTREE_MART_CLERK},
            {MAP_GROUP(HOENN_MOSSDEEP_CITY_MART), MAP_NUM(HOENN_MOSSDEEP_CITY_MART), LOCALID_MOSSDEEP_MART_CLERK},
            {MAP_GROUP(HOENN_SOOTOPOLIS_CITY_MART), MAP_NUM(HOENN_SOOTOPOLIS_CITY_MART), LOCALID_SOOTOPOLIS_MART_CLERK},
            {MAP_GROUP(HOENN_BATTLE_FRONTIER_MART), MAP_NUM(HOENN_BATTLE_FRONTIER_MART), LOCALID_BATTLE_FRONTIER_MART_CLERK}};
...
{
    if (gSaveBlock1Ptr->lastHealLocation.mapGroup == MAP_GROUP(HOENN_DEWFORD_TOWN) && gSaveBlock1Ptr->lastHealLocation.mapNum == MAP_NUM(HOENN_DEWFORD_TOWN))
